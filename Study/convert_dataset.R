...
clean_data <- data %>%
  # 1. Clean DOIs first so we can filter accurately
  mutate(
  ...
  # 4. Clean and filter the outcome_quote
  mutate(
  ...
  # 5. Map the final variables for the game
  mutate(
...
# CRITICAL FIX: Remove any studies where abstract is NA, very short, or starts with "Significance"
